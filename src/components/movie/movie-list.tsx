...
export default function MovieList() {
  ...
  useEffect(() => {
    fetch(`${TMDB_API_URL}${search && search.trim() ?
  ...
  return (
    <>
      <MovieShow selectedMovie={selectedMovie} setSelectedMovie={setSelectedMovie} />
                ...
                <p className="text-xs mt-2 group-hover:duration-500 opacity-0 hidden group-hover:block group-hover:transition-opacity group-hover:opacity-100 absolute group-hover:relative">
                  {
                    movie.overview.length > 100
                      ? `${movie.overview.slice(0, 100).split('.').join('\n')}...`
                      : movie.overview
                  }
                </p>
              </div>
